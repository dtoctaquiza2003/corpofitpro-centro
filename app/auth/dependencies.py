...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> Usuario:
    """
    Dependencia que obtiene el usuario autenticado actual a partir del token JWT.
    """
...
def get_current_terapeuta(current_user: Usuario = Depends(get_current_user)) -> Usuario:
    """
    Verifica que el usuario autenticado sea un terapeuta (rol = 2).
    """
...
def get_current_jefe(current_user: Usuario = Depends(get_current_user)) -> Usuario:
    """
    Verifica que el usuario autenticado sea un jefe (rol = 3).
    """
...
def get_current_secretary(current_user: Usuario = Depends(get_current_user)) -> Usuario:
    """
    Permite secretario o jefe.
    El secretario debe tener consultorio asignado.
    """
